...
import 'package:wms_deal_ncs/pages/delivery%20order/delivery_order_detail.dart';
...
class _DeliveryOrderPageState extends State<DeliveryOrderPage> {
    ...
    if (JwtDecoder.isExpired(token)) {
      ...
      try {
        List<DeliveryOrderModel> data =
            await DeliveryOrderProvider().getDeliveryOrders(
          apikey: keyApi,
          token: token,
          noDo: "",
  ...
  Widget build(BuildContext context) {
    ...
    TextEditingController tanggalController =
        TextEditingController(text: dateDelivered);
                  ...
                  children: [
                    Expanded(
                      child: DateTimeField(
                        ...
                        onChanged: (text) {
                          setState(() {
                            dateDelivered = DateFormat("yyyy-MM-dd")
                                .format(text!)
                                .toString();
                          });
                        ...
                        (data) => GestureDetector(
                          onTap: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => DeliveryOrderDetail(
                                  data,
                                  '',
                                  reload,
                                ),
